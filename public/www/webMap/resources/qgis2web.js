...
map.getView().fit([1113788.895199, 4412072.282025, 1118897.676853, 4414866.025820], map.getSize());
            ...
            if (doPopup) {
                ...
                for (var i=0; i<2; i++) {
                    if (currentFeatureKeys[i] != 'geometry') {
                        ...
                        popupField += '<th><strong>ID:&nbsp;&nbsp;</strong></th><td colspan="2">';
...
var onSingleClick = function(evt) {
    ...
    map.forEachFeatureAtPixel(pixel, function(feature, layer) {
        ...
        let selectID = document.getElementById('Type')
        if(inputID && selectID){
            var evt = document.createEvent("HTMLEvents");
             evt.initEvent("change", false, true);

            if(feature.values_.Layer == "Habitat"){
                selectID.value = "3";
            }else if (feature.values_.type == "route"){
                selectID.value = "2";
            }else if (feature.values_.type == "piste"){
                selectID.value = "1";
            }else if (feature.values_.etat){
                selectID.value = "0";
            }
            selectID.dispatchEvent(evt);
            inputID.value = feature.values_.OBJECTID;
            inputID.dispatchEvent(evt);
        }
            ...
            if (typeof clusteredFeatures !== "undefined") {
                ...
                if (doPopup) {
                    ...
                    for (var i=0; i<currentFeatureKeys.length; i++) {
                        ...
                        if (currentFeatureKeys[i] == "OBJECTID" || currentFeatureKeys[i] == "Layer" || currentFeatureKeys[i] == "Elevation" || currentFeatureKeys[i] == "Shape_Area"
                        || currentFeatureKeys[i] == "nom" || currentFeatureKeys[i] == "numero" || currentFeatureKeys[i] == "etat"
                        || currentFeatureKeys[i] == "type" || currentFeatureKeys[i] == "type" || currentFeatureKeys[i] == "direction"|| currentFeatureKeys[i] =="SHAPE_Leng"
                        || currentFeatureKeys[i] == "type" || currentFeatureKeys[i] == "type" || currentFeatureKeys[i] == "direction" || currentFeatureKeys[i] =="SHAPE_Leng") {
                            popupField = '';
                                ...
                                if(currentFeature.get(currentFeatureKeys[i]) != null) popupField += '<strong>' + layer.get('fieldAliases')[currentFeatureKeys[i]] + ':</strong>';
                            
                            if (layer.get('fieldImages')[currentFeatureKeys[i]] != "ExternalResource") {
                                popupField += (currentFeature.get(currentFeatureKeys[i]) != null ? '&nbsp&nbsp' +autolinker.link(currentFeature.get(currentFeatureKeys[i]).toLocaleString()) + '</td>' : '');
                            }
                            popupText += '<tr>' + popupField + '</tr>';
                        }
                    }
